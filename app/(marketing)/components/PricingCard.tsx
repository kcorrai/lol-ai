    ...
    <div
      className={cn(
        ...
        isPro
          ...
          : isTeam
            ? "border-accent/50 bg-gradient-to-b from-accent/8 via-surface to-surface shadow-xl shadow-accent/10"
    ...
    >
      ...
      <div className="mb-6">
        <p className={cn(
          ...
          isPro ? "text-accent" : isTeam ? "text-accent" : "text-text-muted"
        )}>
          {name}
        </p>
        {/* Three columns arrive at md, where each card is only ~230px wide — a
            48px price plus its period overflows there, so it scales up at lg. */}
        <div className="mt-3 flex flex-wrap items-baseline gap-x-1">
          <span className="font-display text-4xl font-bold text-text lg:text-5xl">{price}</span>
        ...
        {features.map((f) => (
          <li key={f.label} className="flex items-start gap-2.5 text-sm">
            <Check className={cn(
              ...
              isPro ? "text-accent" : isTeam ? "text-accent" : "text-success"
            )} />
      ...
      {(isPro || isTeam) && proFeatures && proFeatures.length > 0 && (
        <>
          <div className="my-5 flex items-center gap-3">
            <div className={cn("h-px flex-1", isPro ? "bg-accent/20" : "bg-accent/20")} />
            <span className={cn(
              "text-[11px] font-semibold uppercase tracking-widest",
              isPro ? "text-accent/70" : "text-accent/70"
            )}>
