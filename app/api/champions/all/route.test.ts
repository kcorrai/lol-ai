...
describe("GET /api/champions/all", () => {
  it("returns the champion catalogue", async () => {
    const res = await GET(routeRequest("/api/champions/all"));

    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ data: CHAMPIONS });
  });

  /**
   * The catalogue changes at most once per patch. Re-reading it from Postgres on every request is
   * the read amplification that exhausted the Neon transfer quota (TASK-282).
   */
  ...
  it("rate limits by IP", async () => {
    await GET(routeRequest("/api/champions/all", { headers: { "x-forwarded-for": "1.2.3.4" } }));

    expect(vi.mocked(checkRateLimit).mock.calls[0][0]).toBe("champions-all:1.2.3.4");
  });
