...
| **`@tauri-apps/plugin-autostart`** | 2.5.1            | **Added for the tray work.** The webview half of `tauri-plugin-autostart` — three calls that read or set one boolean about this app alone. The Settings switch reads the value back from the operating system after every write rather than remembering what it asked for, because a refused write must not leave a control claiming a setting it did not get. |
...
| Crate                     | Version                             | Why                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------- | ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tauri`                   | 2.11, `tray-icon`                   | The shell. Chosen over Electron because this app runs _while League runs_: it renders through the OS webview instead of shipping a second browser (ADR-038). The `tray-icon` feature is what lets it keep running with no window on screen, which is the difference between a companion that is watching when a game starts and one that is not.                                                                                                                                                                                                                                                           |
...
| **`tauri-plugin-autostart`** | 2.5 | **Added for the tray work.** Puts the app in the machine's start-up list, and only when the player asks in Settings — it ships off. A companion that has to be launched by hand before a match is one that is not running when the match starts, which is the whole reason the tray work exists. Writing to the registry Run key (or a LaunchAgent, or an autostart desktop entry) by hand is three platform-specific things to get wrong. |
| **`tauri-plugin-single-instance`** | 2.4 | **Added for the tray work.** A second copy would poll `2999` alongside the first and open its own handle on the credential store. Registered before every other plugin, because the mechanism is that the second process hands over its arguments and exits — anything registered earlier would run in the process that is about to die. Launching the app again is how a player asks for the window back from the tray, so that is what the hook does. |
| **`tauri-plugin-global-shortcut`** | 2 | **Added for the overlay.** One combination, `Ctrl+Alt+L`, registered in Rust and never granted to the webview — a renderer able to claim arbitrary global shortcuts could take keys away from the game. It has to be global because the overlay must be reachable while the game holds the keyboard. This listens for a key; it never sends one, which is the line ADR-038 draws against input injection. A registration that fails warns and carries on, because the tray menu reaches the overlay too. |
...
| **`getrandom`, `sha2`, `hex`** | 0.2 / 0.10 / 0.4 | **Added for ADR-048.** The app generates a pairing secret, sends the website only its SHA-256, and later presents the secret itself to claim its device token — so it needs a source of random bytes and one hash function. All three were already in the tree beneath `reqwest` and `rustls`; naming them makes them the app's own rather than borrowed from a crate that could drop them tomorrow. Nothing is added to the build. `getrandom` over `rand` because the whole need is thirty-two bytes from the operating system, and `rand`'s generators and distributions are not wanted here.                                                                                                                                                                                                                                                                                                                                                                              |
