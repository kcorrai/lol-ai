...
plainly in its help centre: _"no application, no waitlist, and no vetting."_ A
...
it requires a production API key _and_ a separate invitation, and the odds and
...
_"we read this rank from Riot and it is dated"_, not _"Riot confirmed this is
their account"_ — and even the first is more than anyone else in the category
offers.
...
| `PLATFORM_CHECKED` | the rank on a linked account, as we read it, dated and refreshed daily | "Rank checked by LaneIQ · _date_" | **now**                      |
