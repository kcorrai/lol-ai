...
(function () {
  ...
  const HANDLES = [
    ["Riftwalker", "NA1"], ["saltminer", "EUW"], ["azirmain", "TR1"], ["clickhead", "NA1"],
    ["ok boomer", "EUW"], ["hardstuck", "TR1"], ["ping9", "NA1"], ["freeloss", "EUW"],
    ["notaint", "TR1"], ["dc_pls", "NA1"],
  ];
  ...
  BLUE_CHAMPS.forEach((champ, i) => {
    participants.push({
      puuid: `puuid-blue-${i}`, championName: champ, teamId: 100, position: POSITIONS[i],
      gameName: HANDLES[i][0], tagLine: HANDLES[i][1],
    });
  });
  RED_CHAMPS.forEach((champ, i) => {
    participants.push({
      puuid: `puuid-red-${i}`, championName: champ, teamId: 200, position: POSITIONS[i],
      gameName: HANDLES[i + 5][0], tagLine: HANDLES[i + 5][1],
    });
  });
  ...
  for (let m = 0; m <= DURATION_MIN; m++) {
    ...
    const players = participants.map((p, i) => {
      ...
      const totalGold = Math.max(500, Math.round(500 + m * roleGoldRate + side * arc * 0.4 + variance));
      const level = Math.min(18, Math.max(1, Math.round(1 + m * 0.52 + (role === "UTILITY" ? -1.5 : 0))));
  ...
  function addEvent(minute, kind, actorPuuid, payload, posSeed) {
    ...
    events.push({
      kind, timestampMs: minute * 60000 + Math.round(rand() * 50000), minute, actor, position, payload,
    });
  }

  // Turret plates, pre-14m only (that's the real game rule).
  [[4, "red-3"], [6, "blue-2"], [9, "red-1"], [11, "blue-3"], [12, "red-0"], [13, "blue-1"]].forEach(
    ([m, actor]) => {
      const teamId = actor.startsWith("blue") ? 100 : 200;
      addEvent(m, "TURRET_PLATE_DESTROYED", `puuid-${actor}`, {
        teamId, laneType: pick(["TOP_LANE", "MID_LANE", "BOT_LANE"]),
      });
    }
  );
  ...
  function skirmish(minute, killerSide, positions) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      addEvent(minute, "CHAMPION_KILL", victim.puuid, {
        killerId: null, killerPuuid: killer.puuid,
        assistingParticipantIds: [], bounty: 150 + Math.round(rand() * 150),
      }, positions[i]);
    }
    if (n >= 2) {
      addEvent(minute, "CHAMPION_SPECIAL_KILL", killers[0].puuid, {
        killType: "KILL_MULTI", multiKillLength: n,
      }, positions[0]);
    }
  }
  skirmish(9, 200, [[7400, 8600], [7900, 8100]]);
  skirmish(18, 100, [[9200, 10400], [9700, 9900], [10100, 9400]]);
  skirmish(27, 100, [[11200, 4800], [10800, 5200]]);

  // A handful of solo kills scattered elsewhere.
  [[6, 100], [13, 200], [21, 200], [24, 100], [30, 100]].forEach(([m, side]) => {
    ...
    addEvent(m, "CHAMPION_KILL", victim.puuid, {
      killerId: null, killerPuuid: killer.puuid, assistingParticipantIds: [], bounty: 300,
    });
  });

  // Objectives.
  addEvent(9, "ELITE_MONSTER_KILL", "puuid-red-1", { monsterType: "DRAGON", monsterSubType: "INFERNAL_DRAGON", killerTeamId: 200 }, [8100, 3300]);
  addEvent(19, "ELITE_MONSTER_KILL", "puuid-blue-1", { monsterType: "RIFTHERALD", monsterSubType: null, killerTeamId: 100 }, [10300, 8700]);
  addEvent(22, "ELITE_MONSTER_KILL", "puuid-blue-1", { monsterType: "DRAGON", monsterSubType: "INFERNAL_DRAGON", killerTeamId: 100 }, [8100, 3300]);
  addEvent(29, "ELITE_MONSTER_KILL", "puuid-blue-1", { monsterType: "BARON_NASHOR", monsterSubType: null, killerTeamId: 100 }, [4900, 10600]);

  // Buildings — from ~14m on, blue eventually closes it out.
  [[14, 200, "TOP_LANE"], [16, 100, "BOT_LANE"], [20, 200, "MID_LANE"], [25, 100, "TOP_LANE"],
   [28, 100, "MID_LANE"], [31, 100, "BOT_LANE"], [33, 100, "MID_LANE"]].forEach(([m, victTeam, lane]) => {
    addEvent(m, "BUILDING_KILL", null, {
      teamId: victTeam === 100 ? 200 : 100, buildingType: "TOWER_BUILDING", laneType: lane,
