...
(function () {
  ...
  var KIND_STYLE = {
    ...
    TURRET_PLATE_DESTROYED: { shape: "hollowSquare", color: "var(--red-500)", label: "Plate", op: 0.65 },
  ...
  var state = { minute: 18, playing: false, speed: 1, activeKinds: new Set(KIND_ORDER), timer: null };

  var byMinute = new Map(DATA.frames.map(function (f) { return [f.minute, f]; }));
  function maxAbsDiff() {
    return DATA.frames.reduce(function (m, f) { return Math.max(m, Math.abs(f.teamGoldDiff)); }, 1);
  }
  var MAXDIFF = maxAbsDiff();

  function activeEvents() {
    return DATA.events.filter(function (e) { return state.activeKinds.has(e.kind); });
  }
  function eventsAt(minute) {
    return activeEvents().filter(function (e) { return e.minute === minute; });
  }
  ...
  function xFor(minute) { return VB.ml + (minute / DURATION) * PW; }
  function yFor(diff) { return baseY - (diff / MAXDIFF) * (PH / 2 - 6); }
  ...
  function shapeNode(kind, cx, cy, r, opacity) {
    ...
    var g = el("g", { opacity: (s.op || 1) * opacity, transform: "translate(" + cx + "," + cy + ")" });
    var fillProps = { fill: s.color };
    var strokeProps = { fill: "none", stroke: s.color, "stroke-width": 1.4 };
    switch (s.shape) {
      case "circle": g.appendChild(el("circle", Object.assign({ r: r }, fillProps))); break;
      case "ring":
        g.appendChild(el("circle", Object.assign({ r: r }, fillProps)));
        g.appendChild(el("circle", Object.assign({ r: r + 2.5 }, strokeProps)));
        break;
      case "diamond":
        g.appendChild(el("polygon", Object.assign({ points: pt([[0, -r], [r, 0], [0, r], [-r, 0]]) }, fillProps)));
        break;
      case "square": g.appendChild(el("rect", Object.assign({ x: -r, y: -r, width: r * 2, height: r * 2 }, fillProps))); break;
      case "hollowSquare": g.appendChild(el("rect", Object.assign({ x: -r, y: -r, width: r * 2, height: r * 2 }, strokeProps))); break;
      case "triangle": g.appendChild(el("polygon", Object.assign({ points: pt([[0, -r], [r, r], [-r, r]]) }, fillProps))); break;
      case "hollowTriangle": g.appendChild(el("polygon", Object.assign({ points: pt([[0, -r], [r, r], [-r, r]]) }, strokeProps))); break;
    }
    return g;
  }
  function pt(pairs) { return pairs.map(function (p) { return p[0] + "," + p[1]; }).join(" "); }
  ...
  function renderChart() {
    ...
    var clipTop = "clipTop", clipBot = "clipBot";
    var defs = el("defs", {});
    ...
    for (var m = 0; m <= DURATION; m += 5) {
      var gx = xFor(m);
      chartSvg.appendChild(el("line", { x1: gx, x2: gx, y1: VB.mt, y2: VB.h - VB.mb, stroke: "rgba(255,255,255,0.05)" }));
      var lbl = el("text", { x: gx, y: VB.h - 8, "font-size": 9, fill: "var(--fg-4)", "text-anchor": "middle", "font-family": "JetBrains Mono, monospace" });
      lbl.textContent = m + "m";
      chartSvg.appendChild(lbl);
    }
    // zero baseline
    chartSvg.appendChild(el("line", { x1: VB.ml, x2: VB.w - VB.mr, y1: baseY, y2: baseY, stroke: "var(--line-3)", "stroke-dasharray": "3 3" }));
    var blueLbl = el("text", { x: VB.w - VB.mr, y: VB.mt + 10, "font-size": 9, fill: "var(--blue-500)", "text-anchor": "end", "font-family": "JetBrains Mono, monospace" });
    blueLbl.textContent = "BLUE AHEAD";
    chartSvg.appendChild(blueLbl);
    var redLbl = el("text", { x: VB.w - VB.mr, y: VB.h - VB.mb - 4, "font-size": 9, fill: "var(--red-500)", "text-anchor": "end", "font-family": "JetBrains Mono, monospace" });
    ...
    var pts = DATA.frames.map(function (f) { return [xFor(f.minute), yFor(f.teamGoldDiff)]; });
    var areaPath = "M" + pts.map(function (p) { return p[0] + "," + p[1]; }).join(" L")
      + " L" + xFor(DURATION) + "," + baseY + " L" + xFor(0) + "," + baseY + " Z";
    chartSvg.appendChild(el("path", { d: areaPath, fill: "var(--blue-500)", opacity: 0.16, "clip-path": "url(#" + clipTop + ")" }));
    chartSvg.appendChild(el("path", { d: areaPath, fill: "var(--red-500)", opacity: 0.16, "clip-path": "url(#" + clipBot + ")" }));
    var linePath = "M" + pts.map(function (p) { return p[0] + "," + p[1]; }).join(" L");
    chartSvg.appendChild(el("path", { d: linePath, fill: "none", stroke: "var(--fg-1)", "stroke-width": 1.5 }));

    // clustered event ticks along the top lane
    var byMin = {};
    activeEvents().forEach(function (e) { (byMin[e.minute] = byMin[e.minute] || []).push(e); });
    Object.keys(byMin).forEach(function (mk) {
      var mm = Number(mk), list = byMin[mk];
      var tx = xFor(mm), ty = 16;
      var g = shapeNode(list[0].kind, tx, ty, 4, mm <= state.minute ? 1 : 0.3);
      g.style.cursor = "pointer";
      g.addEventListener("click", function () { setMinute(mm); });
      chartSvg.appendChild(g);
      if (list.length > 1) {
        var badge = el("text", { x: tx + 7, y: ty - 5, "font-size": 8.5, fill: "var(--fg-2)", "font-family": "JetBrains Mono, monospace" });
    ...
    chartSvg.appendChild(el("line", { x1: px, x2: px, y1: VB.mt - 6, y2: VB.h - VB.mb, stroke: "var(--acid-500)", "stroke-width": 1.5 }));
    chartSvg.appendChild(el("polygon", { points: pt([[px - 5, VB.mt - 6], [px + 5, VB.mt - 6], [px, VB.mt + 2]]), fill: "var(--acid-500)" }));
  }
  ...
  function renderReadout() {
    ...
    readout.textContent = (diff >= 0 ? "+" : "") + Math.abs(diff).toLocaleString() + "g " + side + " · " + state.minute + "m";
  ...
  function renderMap() {
    ...
    var visible = activeEvents().filter(function (e) { return e.position && e.minute <= state.minute; });
  ...
  function describeEvent(e) {
    ...
    switch (e.kind) {
      case "CHAMPION_KILL": return (actor ? actor : "Someone") + " was taken down";
      case "CHAMPION_SPECIAL_KILL": return (actor ? actor : "Someone") + " — " + (e.payload.multiKillLength || 2) + "-kill";
      case "ELITE_MONSTER_KILL": return (e.payload.monsterType || "Objective").replace(/_/g, " ") + " taken";
      case "BUILDING_KILL": return (e.payload.laneType || "").replace("_LANE", "") + " " + (e.payload.towerType || "tower").replace(/_/g, " ").toLowerCase() + " fell";
      case "TURRET_PLATE_DESTROYED": return (e.payload.laneType || "").replace("_LANE", "") + " plate destroyed";
      case "WARD_PLACED": return (actor ? actor : "Someone") + " placed a ward";
      case "WARD_KILL": return (actor ? actor : "Someone") + " cleared a ward";
  ...
  function renderFeed() {
    ...
    list.forEach(function (e) {
      ...
      if (e.minute === state.minute) { li.classList.add("is-current"); currentRow = li; }
      li.style.opacity = e.minute <= state.minute ? decay(e.minute) : 1;
      li.style.cursor = "pointer";
      li.addEventListener("click", function () { setMinute(e.minute); });

      var min = document.createElement("span"); min.className = "fmin"; min.textContent = e.minute + "m";
      ...
      var desc = document.createElement("span"); desc.className = "fdesc";
      var isSelf = e.actor && e.actor.puuid === DATA.meta.selfPuuid;
      desc.innerHTML = (isSelf ? "<span class=\"fself\">●</span> " : "") + escapeHtml(describeEvent(e));
      var actor = document.createElement("span"); actor.className = "factor";
      actor.textContent = e.actor ? e.actor.position : "";

      li.appendChild(min); li.appendChild(swSpan); li.appendChild(desc); li.appendChild(actor);
  ...
  function renderChips() {
    KIND_ORDER.forEach(function (kind) {
      ...
      chip.addEventListener("click", function () {
        if (state.activeKinds.has(kind)) state.activeKinds.delete(kind); else state.activeKinds.add(kind);
  ...
  function renderAll() { renderChart(); renderReadout(); renderMap(); renderFeed(); }

  function stepToEvent(dir) {
    var minutes = Array.from(new Set(activeEvents().map(function (e) { return e.minute; }))).sort(function (a, b) { return a - b; });
    var target = dir > 0
      ? minutes.find(function (m) { return m > state.minute; })
      : minutes.slice().reverse().find(function (m) { return m < state.minute; });
  ...
  function togglePlay(force) {
    ...
    if (state.playing) {
      state.timer = setInterval(function () {
        if (state.minute >= DURATION) { togglePlay(false); return; }
        setMinute(state.minute + 1);
      }, 1000 / state.speed);
    }
  }

  document.getElementById("scrubber").addEventListener("input", function (e) { setMinute(Number(e.target.value)); });
  document.getElementById("play-btn").addEventListener("click", function () { togglePlay(); });
  document.getElementById("prev-event-btn").addEventListener("click", function () { stepToEvent(-1); });
  document.getElementById("next-event-btn").addEventListener("click", function () { stepToEvent(1); });
