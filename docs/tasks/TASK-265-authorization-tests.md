...
The audit confirmed the _call sites_ apply them consistently across the API surface. The functions
...
- **`assertCanGenerateReport`** — below both caps; at the monthly cap of 3; at the daily cap of 1
  _while the month is not exhausted_ (the daily cap exists precisely so a free user cannot burn the
