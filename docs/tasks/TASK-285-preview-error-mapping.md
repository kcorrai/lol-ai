...
Investigating that surfaced two code defects that turn *every* recoverable
...
2. **`src/domains/riot/services/previewService.ts:96` awaits `setCached`
   unguarded.** The cache *read* is wrapped in try/catch, the *write* is not.
...
- The preview endpoint keeps serving while Neon is down (read *and* write paths).
