  ...
  return (
    <div className="notch absolute left-0 right-0 top-[calc(100%+6px)] z-50 max-h-[380px] overflow-y-auto border border-border bg-surface shadow-[0_12px_32px_rgba(0,0,0,0.55)]">
      {rows.length === 0 ? (
        ...
        // Flat on purpose. An `option` has to be owned by its `listbox`, so the group headings are
        // presentation rows in the same list rather than nested sub-lists — which is what they
        // were, and it hid every option from the accessibility tree.
        <ul id={listId} role="listbox" aria-label="Player search results">
          {groupRows(rows).map((group) => (
            <Fragment key={`${group.rows[0]?.section}-${group.start}`}>
              <li
                role="presentation"
                className="hud-label border-b border-border/60 bg-surface-dark px-3 py-1.5"
              >
              ...
              {group.rows.map((row, offset) => {
                ...
                return (
                  <li
                    ...
                    aria-label={`${row.hit.gameName}#${row.hit.tagLine}`}
