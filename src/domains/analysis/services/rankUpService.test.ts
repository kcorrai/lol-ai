...
describe("winRatePts", () => {
  ...
  it("returns 0 for < 45%", () => expect(winRatePts(0.4)).toBe(0));
});
...
describe("trendFromMatches", () => {
  ...
  it("stable when difference ≤ 15pp", () => {
    ...
    const matches = [
      ...Array(3).fill({ won: true }),
      ...Array(2).fill({ won: false }),
      ...Array(3).fill({ won: true }),
      ...Array(2).fill({ won: false }),
    ];
...
describe("mentalPts", () => {
  ...
  it("returns 12 for 1-loss streak", () =>
    expect(mentalPts([{ won: false }, ...wins(4)])).toBe(12));
...
describe("estimatedGames", () => {
  ...
  it("computes correct estimate for 60% WR at 80 LP", () => {
    ...
    expect(estimatedGames(80, 0.6)).toBe(4);
  });
...
describe("nextLabel", () => {
  it("shows next division within same tier", () =>
    expect(nextLabel("GOLD", "III")).toBe("GOLD II"));
