...
export function CoachingActionsCard({
  onSessionReview,
  onClimbRoadmap,
  onAramReview,
  hasAramMatches,
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      <button
        ...
        className="bg-accent/8 group relative flex items-center gap-4 overflow-hidden rounded-xl border-2 border-accent/50 p-5 text-left transition-all duration-200 hover:border-accent hover:bg-accent/15 disabled:cursor-not-allowed disabled:opacity-50"
        style={{ boxShadow: "0 0 0 0 rgba(198,255,61,0)" }}
        onMouseEnter={(e) => {
          (e.currentTarget as HTMLElement).style.boxShadow = "0 0 20px rgba(198,255,61,0.15)";
        }}
        onMouseLeave={(e) => {
          (e.currentTarget as HTMLElement).style.boxShadow = "0 0 0 0 rgba(198,255,61,0)";
        }}
      >
        <div className="bg-accent/8 pointer-events-none absolute right-0 top-0 h-24 w-24 rounded-full blur-2xl" />
      ...
      >
        <div className="bg-success/6 pointer-events-none absolute right-0 top-0 h-24 w-24 rounded-full blur-2xl" />
        ...
        >
          ...
          <div className="relative">
            ...
            <p className="mt-0.5 text-sm text-text-muted">
              Team fight-focused coaching for your recent ARAM matches
            </p>
          </div>
          <span className="ml-auto shrink-0 rounded-full border border-info/30 px-2 py-0.5 text-[11px] font-semibold text-info">
            ARAM
          </span>
        </button>
      )}
