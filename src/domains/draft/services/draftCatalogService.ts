...
function lanesFor(stats: ChampionMetaStats | undefined): DraftChampion["lanes"] {
  ...
  return ranked
    .filter((p) => p.games >= main.games * SECONDARY_LANE_SHARE)
    .map((p) => p.position);
}
...
export async function getDraftCatalog(nowMs: number = Date.now()): Promise<DraftCatalog> {
  ...
  const [rows, snapshot] = await Promise.all([
    prisma.champion.findMany({ select: { key: true, name: true }, orderBy: { name: "asc" } }),
    getMetaSnapshot().catch(() => null),
  ]);

  const byKey = new Map(
    (snapshot?.champions ?? []).map((c) => [c.championKey.toLowerCase(), c])
  );

  const champions: DraftChampion[] = rows.map((row) => {
    const stats = byKey.get(row.key.toLowerCase());
    return {
      key: row.key,
      name: row.name,
