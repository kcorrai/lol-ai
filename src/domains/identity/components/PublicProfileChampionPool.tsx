      ...
      <div className="space-y-3">
        {champions.map((c, i) => {
          ...
          return (
            ...
            >
              <span className="w-4 shrink-0 text-center text-xs font-bold text-text-muted/40">
                {i + 1}
              </span>
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1.5 h-1 overflow-hidden rounded-full bg-white/5">
                  <div
                    ...
                    style={{ width: `${c.winRate}%`, background: wc }}
                  />
                </div>
                <div className="mt-1.5 flex items-center gap-3 text-[11px] text-text-muted">
                  <span style={{ color: wc }} className="font-semibold">
                    {c.winRate}% WR
                  </span>
                  <span className="flex items-center gap-0.5">
                    <Crosshair className="h-2.5 w-2.5" />
                    <span style={{ color: kc }} className="font-medium">
                      {c.avgKda} KDA
                    </span>
                  </span>
                  <span className="font-medium">{c.avgCsPerMinute} CS/min</span>
                  {c.masteryPoints !== null && (
                    <span className="ml-auto text-warning/60">
                      {formatMasteryPoints(c.masteryPoints)} p
                    </span>
                  )}
                </div>
              </div>
