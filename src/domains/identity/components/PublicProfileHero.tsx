...
interface RankInfo { tier: string; division: string; lp: number; wins: number; losses: number; hotStreak: boolean }
...
export function PublicProfileHero({ displayName, profileIconId, rank, region, joinedAt, featuredChampion, isPrivate }: Props) {
  ...
  return (
    ...
    >
      {splashUrl ? (
        <>
          <Image fill alt="" aria-hidden src={splashUrl} className="object-cover object-[65%_15%]" style={{ filter: "saturate(0.6) brightness(0.3)" }} />
        ...
        <div className="min-w-0 flex-1 pb-1">
          ...
          {rank && !isPrivate ? (
            <div className="mt-1.5 flex flex-wrap items-center gap-2">
              ...
              <span className="text-sm text-text-muted">{rank.wins}W {rank.losses}L</span>
            </div>
          ) : !isPrivate ? (
