...
export function RecommendedOtps({ recommendations, onSelect }: Props): React.JSX.Element | null {
  ...
  return (
    <div
      data-tour="otp-recommendations"
      className="mb-6 rounded-xl border border-accent/30 bg-accent/5 p-4"
    >
      <div className="mb-3 flex items-center gap-1.5">
        <Sparkles className="h-4 w-4 text-accent" />
        <p className="text-xs font-semibold uppercase tracking-widest text-accent">
          Recommended OTPs for you
        </p>
      </div>
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-text-muted">
                {POSITION_LABEL[r.position] ?? r.position} · {r.games} games
              </p>
