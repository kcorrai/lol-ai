...
const freq = (entries: [number, [CanonicalPosition, number][]][]): LaneFrequency =>
  new Map(
    entries.map(([id, lanes]) => [id, lanes.map(([position, games]) => ({ position, games }))])
  );
...
describe("assignLanes", () => {
  ...
  it("reads Smite in either spell slot", () => {
    const second: LiveParticipant = {
      championId: 1,
      teamId: 100,
      spell1Id: FLASH,
      spell2Id: SMITE_SPELL_ID,
    };
  ...
  it("fills both teams from one game", () => {
    const game = [
      player(6, 100),
      player(3, 100, SMITE_SPELL_ID),
      player(1, 100),
      player(4, 100),
      player(5, 100),
      player(6, 200),
      player(3, 200, SMITE_SPELL_ID),
      player(2, 200),
      player(4, 200),
      player(5, 200),
    ];
