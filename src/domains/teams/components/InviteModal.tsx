...
export function InviteModal({ teamId, onClose, onInvited }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error?.message ?? "Davet gönderilemedi");
      }
      onInvited();
      onClose();
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="mb-4 text-lg font-semibold text-text">Üye Davet Et</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-text-muted">Rol</label>
            <select
              value={role}
