...
export function TeamDashboard({ teamId, isCoach }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "comparison" | "stats" | "lineup">(
    "overview"
  );
  ...
  const WrIcon =
    team.avgWinRate7d === null
      ? null
      : team.avgWinRate7d >= 55
        ? TrendingUp
        : team.avgWinRate7d < 45
          ? TrendingDown
          : Minus;
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {team.avgWinRate7d !== null && WrIcon && (
              <div
                className={`flex items-center gap-1 rounded-lg border px-2.5 py-1 text-xs font-semibold ${team.avgWinRate7d >= 55 ? "border-success/30 bg-success/10 text-success" : team.avgWinRate7d < 45 ? "border-danger/30 bg-danger/10 text-danger" : "border-border bg-surface-2 text-text-muted"}`}
              >
      ...
      <div className="flex gap-1 rounded-lg border border-border bg-surface-2 p-1 text-sm">
        ...
        >
          <BarChart2 className="mr-1.5 inline h-3.5 w-3.5" />
          Comparison
        </button>
        <button
          ...
          <div className="space-y-2">
            {members.map((member) => (
              <TeamMemberCard
                ...
                onRoleChange={
                  isCoach ? (uid, role) => roleM.mutate({ userId: uid, role }) : undefined
                }
              />
            ))}
          </div>
      ...
      {activeTab === "lineup" && (
        <TeamLineup teamId={teamId} members={members} canManage={isCoach} />
      )}
