...
describe("checkRateLimit", () => {
  ...
  it("returns correct Retry-After time", () => {
    ...
    checkRateLimit(key, config, now);
    const blocked = checkRateLimit(key, config, now + 1_000);
  ...
  it("resets the window after windowMs elapses", () => {
    ...
    const first = checkRateLimit(key, config, now);
    expect(first.allowed).toBe(true);

    const blocked = checkRateLimit(key, config, now + 500);
    expect(blocked.allowed).toBe(false);

    const afterReset = checkRateLimit(key, config, now + 1_001);
  ...
  it("tracks different keys independently", () => {
    ...
    const a = checkRateLimit(`rl-test-keys-a:${ts}`, config, now);
    const b = checkRateLimit(`rl-test-keys-b:${ts}`, config, now);

    expect(a.allowed).toBe(true);
    expect(b.allowed).toBe(true);
  });

  it("reports the correct limit in all responses", () => {
    const key = `rl-test-meta:${Date.now()}`;
    const config = { limit: 5, windowMs: 60_000 };
    const now = Date.now();

    const r = checkRateLimit(key, config, now);
